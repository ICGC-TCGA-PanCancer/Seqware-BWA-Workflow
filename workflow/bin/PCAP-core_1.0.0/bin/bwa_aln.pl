...

{
  ...
  $threads->add_function('bam_to_grouped_bam', \&PCAP::Bam::bam_to_grouped_bam);
  $threads->add_function('sampe', \&PCAP::Bwa::sampe, 3);

  my $bam_count = scalar @{$options->{'meta_set'}};
  if(!exists $options->{'process'} || $options->{'process'} eq 'bam2fq') {
    $threads->run($bam_count, 'bam_to_grouped_bam', $options) unless($options->{'meta_set'}->[0]->fastq);
  }
  PCAP::Bwa::bwa_aln($options) if(!exists $options->{'process'} || $options->{'process'} eq 'aln');
  $threads->run($bam_count, 'sampe', $options) if(!exists $options->{'process'} || $options->{'process'} eq 'sampe');
